...
- **Ativação**:
  - **Lógica Exata**: Coletar 3 itens do tipo "Música" (Karaokê, Nota Musical, Pagode) consecutivamente (sem coletar outros tipos entre eles) OU atingir combo x3.0 e mantê-lo por 5 segundos.
- **Efeitos**:
  - Contagem regressiva de duração (10s)
  - Som de fundo agitado ("fever")
  - Efeitos visuais intensificados (fundo piscando)
...
### Modelos Disponíveis
1. **Homem**: Corpo padrão masculino (Renderizado vetorialmente via `ctx.fillRect/arc`)
2. **Mulher**: Corpo feminino com cabelo longo (Renderizado vetorialmente via `ctx.fillRect/arc`)
3. **Sprite Custom**: Suporte para sprite sheet externo (48x48px, 3 linhas x 8 frames)
   - **Mapeamento**:
     - Linha 0 (Y=0): IDLE (4 frames)
     - Linha 1 (Y=48): RUN (8 frames)
     - Linha 2 (Y=96): JUMP (1 frame estático ou loop de 4 frames)

### Customização Procedural (Modelos Padrão)
- **Método**: Desenho vetorial direto no Canvas (não usa tinting de sprites).
...
### 🍺 Sistema de Embriaguez (Drunk System)
- **Mecânica Central**: O charme do jogo. O personagem fica progressivamente "bêbado".
- **Itens**:
  - **Álcool** (Cerveja, Chopp, Coquetel): Aumenta o nível de embriaguez.
  - **Comida/Água**: Diminui o nível de embriaguez (recupera sobriedade).
- **Efeitos Visuais**:
  - Tela balança (sway) e gira levemente proporcional ao nível.
  - Desfoque (blur) em níveis altos.
- **Efeitos de Gameplay**:
  - Dificuldade de input em níveis críticos (chance de delay ou falha no pulo).
  - Decay natural: A embriaguez diminui lentamente com o tempo.
...
**Hitbox (Colisão)**:
- **Lógica**: A hitbox deve ser ligeiramente menor que o visual para ser "justa" (fair play).
- **Largura**: `ctx.measureText(text).width * 0.8` (80% da largura visual)
- **Altura**: 30px (fixa, centralizada verticalmente no texto)
...
- **Comportamento**: Move-se na direção do jogador (da direita para esquerda) com velocidade `speed + 2`.
  - *Exemplo*: Se o chão move a 5px/frame, o garçom move a 7px/frame (aproximação rápida).
...
| FESTEIRO | 50 | Rosa (#FF1493) | +10% Duração Power-ups |
| VIP GOLD | 100 | Dourado (#FFD700) | Inicia com Escudo Ativo |
| NEON | 150 | Ciano (#00FFFF) | 2x Moedas, +10% Velocidade (Hard) |
| FLAMEJANTE | 200 | Laranja (#FF4500) | - |
| GELO | 250 | Azul claro (#87CEEB) | - |
| ARCO-ÍRIS | 300 | - | 2x Moedas, +10% Velocidade, ✨ Gradiente |
| LENDÁRIO | 500 | Roxo (#9370DB) | 👑 Premium (Status Symbol) |
...
- **Funcionalidade**:
  - Gera dinamicamente uma imagem (Canvas 600x300px) com:
    - **Fundo**: Gradiente Vinho (#8B0000) para Escuro (#2c0000)
    - **Borda**: Dourada (#FFD700), 10px
    - **Decoração**: Círculos estilo confete (transparência 0.2)
    - **Título**: "🎫 INGRESSO VIP 🎫" (Courier New, 30px, Dourado, Y=50)
    - **Nome**: Nome do Jogador (Arial, 40px, Branco, Y=110)
    - **Recorde**: "RECORDE: X PONTOS" (Arial, 25px, Dourado, Y=150)
    - **Info**: Data e Local (Arial, 18px, Cinza, Y=200/225)
    - **Desafio**: "Eu vou! Você consegue me superar?" (Arial Italic, 20px, Verde, Y=270)
...
- **Orientação**: Ao detectar mudança de orientação (portrait <-> landscape):
  - Recalcular `groundY` imediatamente.
  - Reposicionar `player.y` para evitar que caia no limbo ou fique flutuando.
  - Pausar o jogo automaticamente (`isPaused = true`) para o jogador se reorientar.
